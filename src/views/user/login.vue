<template>
  <div class="main">
    <a-form id="formLogin" layout="vertical" class="user-layout-login">
      ...
      <a-form-item>
        <a-checkbox v-model:checked="modelRef.rememberMe">自动登录</a-checkbox>
  ...
  setup() {
    ...
    const requestFailed = (err: RequestError) => {
      ...
      notification.error({
        message: "错误",
        description:
          ((err.response || {}).data || {}).errorMessage || "请求出现错误，请稍后再试",
        duration: 4,
      });
    };
    ...
    const isIE = ()=>{
      var version = navigator.userAgent.substr(navigator.userAgent.indexOf('Edge'))
