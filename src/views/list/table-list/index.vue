...
export default defineComponent({
  ...
  setup() {
    ...
    const defaultCheckedList= ['action','name']
    const {
      state: columnState,
      dynamicColumns,
      dynamicColumnItems,
      ...
      reset,
      move,
    } = useTableDynamicColumns(mergedColumns,defaultCheckedList, { needRowIndex });
    ...
    //localForage
   /* localforage.setItem(router.currentRoute.value.name+'checkList',[1,2])
    localforage.getItem(router.currentRoute.value.name+'checkList').then(res=>{
      console.log(res)
    })*/
